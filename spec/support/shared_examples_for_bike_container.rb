...
shared_examples_for BikeContainer do
  ...
  describe 'add_bike' do
    ...
    it 'receives a bike' do
      subject.add_bike(bike_double)
      expect(subject).not_to be_empty
    end

    it 'raises an error when full' do
      subject.capacity.times { subject.add_bike(bike_double) }
      expect { subject.add_bike(bike_double) }.to raise_error "#{described_class.name} full"
    end
  end
